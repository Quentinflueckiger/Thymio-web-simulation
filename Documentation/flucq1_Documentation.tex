...
MOBOTS being the Miniature Mobile Robots Group, they are mainly focused around system design for small robots of the kind. The project started with a strange pile of components, 
...
Every step of the Thymio project is open-source and has a non-profit aim, namely to enhance the quality of it with the user's project and research, 
...
We decided to divide the architecture into two different parts. One for the simulation side and the other one for the customization, although they are very similar there are still some differences that push us to look at them with two different angles. 
Both are build based on a \texttt{Model} \texttt{View} \texttt{Controller} system where the elements of the playground, be it a wall or the Thymio robot, are models and the page seen by the user is the \texttt{View}. This \texttt{View} registers user inputs and transmits them to the \texttt{Controller}. 
...
We decided to load only once the Thymio model, as it takes in average 500ms to 1'000ms to load it, and we encountered some issues where the model would not load everytime the playground was changed.
