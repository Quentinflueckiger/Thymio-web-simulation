...
%%\section{Model View Controller \label{mvc}}
%% TODO : STUFF
%%As described in the 
...
Afterward, it controls that no constant with this name already exists, then controls that the next token is of type \texttt{TOKEN_ASSIGN} to respect how a constant is declared, get the value that should be at the next token, if that one is not of type \texttt{TOKEN_INT_LITERAL} it will return the value 0. 
...
Not every parse methods were translated from the \texttt{C++} program as their amount is rather big and not easy to translate into \texttt{JavaScript}. Below is a diagram that will show the process of a token of type \texttt{TOKEN_STR_onevent} to ease the understanding.
\begin{center}
  \includegraphics[scale=1]{./event_when}
\end{center}
...
Or if the token is of type \texttt{TOKEN_STR_when} it will try to parse the condition and thus will start with the following \texttt{parseOr} method.
...
%% TODO : FILL
%% We saw that Thymio has some pre-programmed behavior directly into him, decided to code one (explorator) and add another one (track follower)
\chapter{Conclusion}
...
For the \texttt{Aseba} possibility we kindly recommend you to use the tutorials and example that can be found on the \hreft{https://www.thymio.org/program/aseba/}{\texttt{Thymio} web page}.

\section{Scratch}

For the \texttt{Scratch} possibility we kindly recommend you to use the tutorials and example that can be found on the \hreft{https://www.thymio.org/program/scratch/}{\texttt{Thymio} web page}.
